...
async function getAccounts(req, res) {
  try {
    ...
    const accounts = await accountsService.getAccounts(status);
...
async function getAccountById(req, res) {
  try {
    ...
    return sendSuccess(res, account, 'Account retrieved successfully', 200);
  } catch (err) {
    const mapped = mapServiceError(err);
    return sendError(res, mapped.message, null, mapped.statusCode);
  }
...
async function reactivateAccount(req, res) {
  try {
    const { id } = req.params;
    const result = await accountsService.reactivateAccount(id);

    return sendSuccess(res, result, 'Account reactivated successfully', 200);
  } catch (err) {
    const mapped = mapServiceError(err);
    return sendError(res, mapped.message, null, mapped.statusCode);
  }
